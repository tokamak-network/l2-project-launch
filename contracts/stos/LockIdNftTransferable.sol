...
contract LockIdNFT is ProxyStorage2, LockIdNFTStorage, LockIdStorage, IERC721, IERC721Metadata, IERC721Enumerable {
    ...
    {
        unlimitedAmountByAccount[account] += amount;
        unlimitedAmountTotal += amount;
        emit IncreasedUnlimitedLock(msg.sender, amount);
    }

    function decreaseUnlimitedLock(uint256 amount)
        public nonZero(amount)
    {
        require(unlimitedAmountByAccount[msg.sender] >= amount, 'unlimitedAmount is insufficient');

        unlimitedAmountByAccount[msg.sender] -= amount;
        unlimitedAmountTotal -= amount;

        uint256 unlockTime = (block.timestamp + maxTime) / epochUnit * epochUnit;
        uint256 lockId = ++maxTokenId;
        _safeMint(msg.sender, lockId);
        _deposit(msg.sender, lockId, amount, unlockTime);
    ...
    function transferFromUnlimited(address from, address to, uint256 amount) public virtual {
        ...
        require (unlimitedAmountByAccount[from] >= amount, "unlimitedAmount is insufficient");
        unlimitedAmountByAccount[from] -= amount;
        unlimitedAmountByAccount[to] += amount;
    }
    ...
    function balanceOfUnlimitedLock()
        public
        view
        returns (uint256 balance)
    {
        return unlimitedAmountTotal / maxTime * maxTime;
    }


    function balanceOfUnlimitedLock(address account)
        public
        view
        returns (uint256 balance)
    {
        return unlimitedAmountByAccount[account] / maxTime * maxTime;
    }
        ...
        returns (uint256 balance)
    {
        ...
        balance += balanceOfUnlimitedLock(_addr);
    }
