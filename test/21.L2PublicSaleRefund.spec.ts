...
describe('L2TokenFactory', () => {
    ...
    let hardcapAmount = ethers.utils.parseUnits("1010", 18);
    let changeTick = 18;
    let changeTOS = 10;
    ...
    let refundAmount = ethers.utils.parseUnits("200", 18);
    ...
    describe("# PublicSale Test", () => {
        ...
        describe("# round1 Sale", () => {
            ...
            it("round1Sale after round1Sale StartTime", async () => {
                ...
                await tonContract.connect(addr1).approve(deployed.l2PublicProxy.address, buyAmount)
                await tonContract.connect(addr2).approve(deployed.l2PublicProxy.address, buyAmount)
                await tonContract.connect(addr3).approve(deployed.l2PublicProxy.address, buyAmount)
                await tonContract.connect(addr4).approve(deployed.l2PublicProxy.address, buyAmount)
                await tonContract.connect(addr5).approve(deployed.l2PublicProxy.address, buyAmount)

                await deployed.l2PublicProxyLogic.connect(addr1).round1Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr2).round1Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr3).round1Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr4).round1Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr5).round1Sale(erc20Atoken.address,buyAmount)
            })
        ...
        describe("# round2 Sale", () => {
            it("can not attend before round2 startTime", async () => {
                await tonContract.connect(addr1).approve(deployed.l2PublicProxy.address, buyAmount)
                let tx = deployed.l2PublicProxyLogic.connect(addr1).round2Sale(erc20Atoken.address,buyAmount)
            ...
            it("can round2Sale after round2Sale StartTime", async () => {
                ...
                await deployed.l2PublicProxyLogic.connect(addr1).round2Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr2).round2Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr3).round2Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr4).round2Sale(erc20Atoken.address,buyAmount)
                await deployed.l2PublicProxyLogic.connect(addr5).round2Sale(erc20Atoken.address,buyAmount)
            })
            ...
            it("check round2 attend (depositAmount check)", async () => {
                ...
                expect(buyAmount).to.be.equal(depositAmount1.depositAmount)
                expect(buyAmount).to.be.equal(depositAmount2.depositAmount)
                expect(buyAmount).to.be.equal(depositAmount3.depositAmount)
                expect(buyAmount).to.be.equal(depositAmount4.depositAmount)
                expect(buyAmount).to.be.equal(depositAmount5.depositAmount)
            })
            ...
            it("openSaleUserAmount check", async () => {
                ...
                let calcultokenAmount = await deployed.l2PublicProxyLogic.calculSaleToken(erc20Atoken.address,buyAmount);
                ...
                expect(addr1round2Amount._realPayAmount).to.be.equal(buyAmount)
                expect(addr2round2Amount._realPayAmount).to.be.equal(buyAmount)
                expect(addr3round2Amount._realPayAmount).to.be.equal(buyAmount)
                expect(addr4round2Amount._realPayAmount).to.be.equal(buyAmount)
                expect(addr5round2Amount._realPayAmount).to.be.equal(buyAmount)
        ...
        describe("# claim", () => {
            ...
            it("If try to claim, addr1 will receive a refund of TON you participated in in the 1st and 2nd rounds", async () => {
                ...
                expect(afterAddr1TONAmount.sub(beforeAddr1TONAmount)).to.be.equal(refundAmount)
            })
            ...
            it("If try to claim, addr2 will receive a refund of TON you participated in in the 1st and 2nd rounds", async () => {
                ...
                expect(afterAddr2TONAmount.sub(beforeAddr2TONAmount)).to.be.equal(refundAmount)
            })
            it("If try to claim, addr3 will receive a refund of TON you participated in in the 1st and 2nd rounds", async () => {
                ...
                expect(afterAddr3TONAmount.sub(beforeAddr3TONAmount)).to.be.equal(refundAmount)
            })
            it("If try to claim, addr4 will receive a refund of TON you participated in in the 1st and 2nd rounds", async () => {
                ...
                expect(afterAddr4TONAmount.sub(beforeAddr4TONAmount)).to.be.equal(refundAmount)
            })
            it("If try to claim, addr5 will receive a refund of TON you participated in in the 1st and 2nd rounds", async () => {
                ...
                expect(afterAddr5TONAmount.sub(beforeAddr5TONAmount)).to.be.equal(refundAmount)
            })
           

            it("PublicSale have 0TON for saleToken", async () => {
                expect(await tonContract.balanceOf(deployed.l2PublicProxy.address)).to.be.equal(0);
            })
        })
